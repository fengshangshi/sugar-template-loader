	...
	request
		...
		.end(function(err, res) {
			...
			res.on('data', function(chunk) {
				stream.write(chunk);
				loaded += chunk.length;
				progress && progress(total ? _.round(loaded/total, 4) : 0, loaded, total);
			});
			...
			res.on('end', function() {
				if (status >= 200 && status < 300 || status === 304) {
					...
					cb(new Error('Request ' + url.href + ': server relay status code ' + status + '.'));
				}
			});
